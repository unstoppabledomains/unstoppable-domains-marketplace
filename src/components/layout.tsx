...
function ExpansionPanel(props) {
	...
	return (
		<div className={`pr-4  `}>
			...
			>
				<Row
					className={` grow  align-middle ${query?.categories === category.category &&
				...
				>
					...
					{hasSubCategories && (
						<div
							className={`self-center   ${isExpanded ? "rotate-180 pl-[8px]" : " pr-[8px]"
								}`}
						>
							<svg
			...
			{open &&
				...
				category.subCategory.map((e) => (
					<div
						className={`pl-5 ${query?.subCategory === e
							? " rounded-[12px] bg-[#8A46FF] pl-[16px] "
							: ""
							}`}
					>
						...
						>
							<p
								className={`text-[16px] font-[500] py-[10px] hover:text-[#fff] capitalize ${query?.subCategory === e
									? " text-white "
									: "text-[#87868C]"
									}`}
							>
	...
	return (
		<Row className="lg:hidden overflow-scroll gap-[16px] py-[32px]">
			{data.data.map((e, index) => (
				...
				>
					<summary
						className={`cursor-pointer bg-[#212026] rounded-[32px] flex justify-between items-center py-[8px] px-[12px] ${query?.categories === e.category
				...
				<aside className="hidden lg:flex md:flex-initial w-3/12 border-r border-r-border-color">
					<div className="w-full">
						<div className="border-b border-b-border-color">
							<div
								className={` align-middle items-center pt-2 pb-2 mr-4 my-2 ${router?.asPath == "/history"
									? " rounded-[12px] bg-[#8A46FF] pl-[16px]"
									: ""
									}`}
							>
						...
						<div
							className={`pt-2 pb-2 mr-4 my-2 ${router?.pathname == "/"
								? " rounded-[12px] bg-[#8A46FF] pl-[16px]"
								: ""
								}`}
						>
